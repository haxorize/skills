...
> **Amended by [ADR-0067](0067-commit-msg-hook-checks-shape-not-register.md):** the one opt-in now enables two git hooks, not one — `setup-hooks.sh` also wires a `commit-msg` hook that blocks a commit, where everything this record decided only warns after a pull.
