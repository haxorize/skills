...
> **Amended by [ADR-0039](0039-work-item-tags-derive-before-prefix.md):** the no-tag-creation constraint below is scoped — it rejected tags as the chart *typing* mechanism, and `Chart-type:` stays the source of truth. The `to-*` publishers now stamp best-effort `System.Tags` at creation, degrading gracefully where creation is denied; `chart-course` still publishes untagged.
