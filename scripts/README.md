# Conventions for `scripts/`, `scripts/git-hooks/` and `.claude/skills/*/scripts/`
...
A new or edited check in any `scripts/*.sh` gate, and a hook under `global/hooks/`, lands with a fixture instance that fires and a mutation that reds its selftest, both ways. Narrowing: an alternative dropped from the pattern, and the instance that stops firing turns the selftest red. Widening: a boundary loosened, and the clean neighbor that starts firing turns it red. So a fixture carries one graded instance per alternative, not one per rule, and the selftest carries the mutation table. The four hook selftests carry none yet. The selftest-pairing check proves a selftest exists, never that it can fail — `security-selftest.sh` graded one instance per rule until 2026-08-29, and cutting `_DROP_HOSTS` from twenty-odd host patterns to one left it green. What can cover the gap mechanically, when a person runs it, is `scripts/mutation-sweep.sh` (nothing schedules it — `sweep-corpus`'s gate loop does not run it): it derives the same roster as `post-merge` plus the libraries, generates one-line mutants of each script (`mutation-sweep.py`'s three operators), runs the paired selftest over each on a copy of the tree, and names every site the selftest stayed green over as SILENT. It is hand-run, not a gate — a full pass costs one selftest run per mutant — so its header sets the cap and the single-target mode, and [ADR-0093](../docs/adr/0093-mutation-sweep-over-the-gates.md) records its first run. A hand-written mutation row still lands with a check; the sweep is the audit that the table is not the only thing holding the selftest honest.

## Landing a change here

`Review required: yes` in `CLAUDE.md`'s `Landing:` block arms the `review-receipt` hook under `global/hooks/` once it is wired in `~/.claude/settings.json` (`bash scripts/install.sh` prints the snippet): a push needs a `/review-changes` report whose `Reviewed-tree:` stamp equals the tree being pushed, and `/address-findings` re-stamps after its fix pass. So the order is review, fix up, commit the whole tree, push — any edit after the last stamp needs a new one. The `pre-commit` git hook runs `lint-skills.sh` and `lint-adrs.sh` on the paths a commit touches, runs the selftest of each staged `scripts/` script or git hook (every selftest when `selftest-lib.sh` is staged; a staged `.claude/skills/*/scripts/` script runs none), and prints a lineage notice for a staged skill listed in `docs/lineage.md`'s **first** table — the second is headed *nothing to diff* and draws none, so a skill carrying only a row there gets no notice; `commit-msg` checks message shape and never register, so `src/committing/references/commit-style.md` is not pruned against it.
