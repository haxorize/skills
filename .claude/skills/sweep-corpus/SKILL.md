...
- `for g in scripts/lint-skills.sh scripts/lint-adrs.sh scripts/*-selftest.sh scripts/git-hooks/*-selftest.sh global/hooks/*-selftest.sh; do echo "== $g"; bash "$g"; echo "exit=$?"; done` — the mechanical gate, verbatim output kept. Every self-test in the repo runs here, derived by the `<script>-selftest.sh` pairing rather than named by hand (the same derivation `scripts/git-hooks/post-merge` makes): a gate nobody runs degrades exactly as silently as no gate. The loop does not short-circuit, because an `&&` chain hides every gate after the first non-zero. Read each `exit=`: 0 is clean, **2 is partial or nothing checked** — a self-test uses it for a run that graded what it could reach and left a branch unexercised, a linter for a tree with nothing to check, `skill-usage.sh` for a count carrying a `+` floor marker — and anything else is a failure. A partial is reported as a partial, never as the sweep's green.
