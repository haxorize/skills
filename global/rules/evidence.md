...
- **Re-measure every count at write time.** A number stated from memory is a guess wearing a number's clothes. If it matters enough to report, it matters enough to run the command again now — and the count comes with the command and its verbatim output, inline, once. That is the whole evidence for a count; no proof blocks, no receipts, no command logs unless asked.
