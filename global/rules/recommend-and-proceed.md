...
A question that occurs to you mid-work is answered yourself and folded in where it is bin 1 or 2; the user hears the decision, not the question. A preference or a contract fork is asked when it is met, since what follows is built on the answer. An outward act is queued, not stopped for: the queued acts are asked once, in the turn that reports the work done, one five-line block per act (an ask names each act — `no-unasked-commits`' rule), each checked first for having already happened so the user is not asked to approve a no-op; an act still queued when the session hands off is listed in the handoff as a question never put.
