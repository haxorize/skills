...
`hooks/` holds hook scripts for the class of failure a hook can see: a tool call whose shape, or whose precondition on disk, is wrong before it runs — and one sensor that sees a turn after it ran. Four ship:
...
- `friction-log.sh` — a `Stop` hook, no matcher: after each agent turn it reads the session transcript from a per-session cursor, and when a user turn since then is correction-shaped ("no, don't", "I said", "from now on", a tool call denied with guidance — the signal table is in `friction-log.py`) it hands the agent one instruction: judge the flagged turns, and append one line per real correction to `~/.claude/friction/log.md` in the shape the `debrief` skill defines. It never writes the log, never grades, and never proposes a fix; `stop_hook_active` is its loop guard; fail-open, no opt-in. The shell file is the wrapper and its header is the contract; the scan is `friction-log.py` beside it, which the wrapper execs and which reads no lib; its selftest checks the line shape against `src/debrief/SKILL.md` byte for byte.

The three `PreToolUse` hooks tokenize the command line through one scanner, `hook-lib.sh` / `hook-lib.py`: how a flag reaches a program — a quote, a wrapper, `bash -c`, `eval`, a shell-fed heredoc, `find -exec`, `xargs`, a variable, a compound statement — is the lib's contract, stated in its header, and each hook holds only its own shape. The two lib files, `selftest-lib.sh`, and `friction-log.sh`'s scanner half `friction-log.py` are not hooks; a hook is the script whose header carries an `# Install note:` line, which is how `install.sh` and the post-merge hook tell them apart, and an `# Event:` line naming the event it is wired to — a hook with none installs as a `PreToolUse` check on Bash, so a `Stop` or `SessionStart` hook written without the line lands on the wrong event. Each hook has a `*-selftest.sh` beside it — the three `PreToolUse` selftests run an expect/reject payload table over the shared `selftest-lib.sh` harness; `friction-log-selftest.sh` reads no lib and runs its own fire/quiet table — run it after changing a rule, and run all three `PreToolUse` selftests after changing a lib.

Hooks are wired in `~/.claude/settings.json`, and **`install.sh` never edits that file**. It prints the snippet; you paste it. **The snippet's path is this checkout** — a `git pull` that edits `hooks/` changes the live hook with no re-consent; the post-merge hook prints one line naming any such change. An *uncommitted* edit under `hooks/` is live the moment it is saved, with no commit, no pull, and no announcement at all — an edit to a lib changes all three hooks at once — so a hook edit runs `bash global/hooks/<hook>-selftest.sh` (an edit to `friction-log.py` is an edit to that hook and runs `friction-log-selftest.sh`), and a lib edit runs all three, before the session's next gated act — including the push of the edit itself.
...
To wire a hook, paste the printed block into `~/.claude/settings.json` under `hooks` (one entry per hook; the other `PreToolUse` hooks take the same shape, and `friction-log` goes under `"Stop"` with no `matcher` key — the installer prints it grouped by event):
