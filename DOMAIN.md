...
| **Work-item tag** | An ADO `System.Tags` value the `to-*` publishers stamp best-effort at creation: the drafted title's leading-bracket contents (the **primary tag**, parsed before `Title prefix:` resolution) unioned with the CLAUDE.md `Additional tags:` line and filtered by `Never tag:`; never reconciled after a title rename, and not applied by `chart-course` (see ADR-0039) | Tag (bare, when the ADO mechanism is meant), Label (GitHub's mechanism) |
