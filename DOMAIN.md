...
| **Completion audit** | The done-claim proof run before declaring an objective complete (`implement`'s close, seeded into `handoff`'s background agents): treat done as unproven, derive the requirements, name the authoritative evidence per requirement, and inspect it at matching scope — a narrow check never supports a broad claim; the audit proves completion rather than failing to find remaining work | Done check (vague), Definition of done (the criteria, not the proof) |
...
| **Declared dependency** | A Behavior skill another skill names in its `requires:` line — an Orchestrator declares the behaviors it delegates to, and a Behavior may declare one it **Lazy load**s; `scripts/install.sh` resolves it so the Prose invocation always finds it. Relaxes ADR-0007 atomicity from "lone skill" to "skill + its declared deps" | Skill import, Require |
...
| **`review-changes`** | The **read-only** judgment review (User-invoked Orchestrator) run before a change lands or on a teammate's PR: it fans **Review lenses** out to subagents (findings only) and never mutates code; resolves its input from the local diff or a PR + a work-item pointer | Conformance-review (earlier name) |
...
| **Reply contract** | `receiving-review`'s rule for PR review threads: every comment gets an outcome reply — "Fixed in `<hash>`" only after the commit exists (thread left open for the reviewer), a reasoned won't-fix (may resolve), or already-addressed (may resolve). A thread is resolved only when the reply legitimately closes it | Outcome reply (the unit; the contract is the rule), Acknowledgment |
...
| **Convergence guard** | The bound on the fix→re-review loop — halt rather than let a review become a rewrite, and file the remainder as follow-ups. Stated in `receiving-review`, the single source of truth; `implement` and `review-changes` point at it | Scope cap (unscoped), Review budget |
...
| **Ship** | The main flow's final step (`which-skill` step 6), owned by the `ship` Skill: carrying a green, reviewed change to a closed **Ticket** — commits in **Commit order**, push, closing comment, plus a branch, PR, and merge where another person must approve. The outbound prose it writes is where unverified claims land — bounded by the **Claims rule** | Deliver (overloaded), Merge (one step of it, and only on the approver path), Close the loop (taken — the mechanical finalize) |
| **Claims rule** | The bound `ship` puts on outbound prose: every assertion in a commit message, PR body, or closing comment is checked against its evidence — scope against the diff, review and coverage claims against `git log <base>..HEAD`, "closes N" against the ticket re-read now — *before* it is drafted. A claim that can't be checked is dropped or weakened, never written | Fact-check (unscoped), Verification pass |
...
| **House commit style** | `ship`'s default shape for commit and PR prose, specified in ship's `commit-style.md`. Overridden only by a convention the project *explicitly declares*, never by undeclared history — the precedence rule lives in the same file | Fallback style (retired — it was briefly the evidence-gated fallback; now the default), Commit convention (the project's declared one, which still wins) |
| **Closing-comment contract** | What a `ship` closing comment must contain: what landed (commit SHAs or PR link), status with every claim checked per the **Claims rule**, the remainder — deliberately-not-done work named plainly — and closure verified by re-reading the ticket state. "Done" alone fails it | Sign-off comment, Completion note |
...
- A **Declared dependency** is a **Behavior skill** an **Orchestrator skill** requires; `scripts/install.sh` resolves it. **Format docs** (inert data — `tracker-resolution.md`, `work-item-tags.md`) stay **Sibling reference files** (ADR-0007); only **behaviors** become **Behavior skills**.
