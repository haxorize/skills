...
| **House commit style** | `ship`'s evidence-gated fallback for commit and PR prose, applied only where the project documents no convention *and* two or three quoted recent subjects prove the history settles nothing: plain-imperative subject (~50 chars, hard 72, no type prefix), a body only when the subject can't carry the why, and a PR description that is the commit body plus links. Project style wins wherever it exists | Default style (unscoped), Commit convention (the project's own, which this never overrides) |
