...
| **Close the loop** | The **mechanical** finalization pass run **once after the slice's behaviors are built and refactored** (not per behavior) — `feedback-loops` runs lint/format/typecheck, triages any red as in- or out-of-scope before fixing it, then migrations and doc updates, resolving commands via CLAUDE.md `## Commands` and deferring stack-specifics to Convention skills. Out-of-scope reds route to `diagnosing-bugs` rather than being absorbed. Judgment review (`review-changes`) and `/simplify` live elsewhere (see below) | Finalize (overloaded), Wrap-up |
...
| **House commit style** | `ship`'s default shape for commit and PR prose: plain-imperative subject (~50 chars, hard 72, no type prefix), a body only when the subject can't carry the why, and a PR description that is the commit body plus links. Overridden only by a convention the project *explicitly declares*, never by undeclared history — the precedence rule lives in ship's `commit-style.md` | Fallback style (retired — it was briefly the evidence-gated fallback; now the default), Commit convention (the project's declared one, which still wins) |
...
| **Work-item tag** | An ADO `System.Tags` value stamped best-effort at creation: the drafted title's leading-bracket contents (the **primary tag**, parsed before `Title prefix:` resolution) unioned with the CLAUDE.md `Additional tags:` line and filtered by `Never tag:`; never reconciled after a title rename, applied by the `to-*` publishers and `chart-course` alike (see ADR-0039) | Tag (bare, when the ADO mechanism is meant), Label (GitHub's mechanism) |
