...
| **Anti-cheat probe** | A `black-box-check` move that distinguishes real work from displayed success: vary the input and watch the output follow, refresh and check the promised persistence, feed invalid input and check the promised handling | Smoke test (different purpose), Sanity check (vague) |
| **Negative-space pass** | `review-changes`' optional closing move on a substantial diff: one fresh subagent reads the vetted findings and hunts only where they did not go — the findings map spent attention; an empty return is a good outcome | Blind-spot pass (upstream name), Second pass (vague) |
...
| **AFK** | "Away-from-keyboard" mode of a Task or Decision ticket — safely driven by the agent alone; granted only when the **Readiness gate**'s four predicates hold, and always paired with a **Stop condition** | Auto, Unattended |
| **Readiness gate** | The four-predicate test (from the `work-item-shape` Behavior) that justifies an **AFK** call: behavior clear to a fresh agent with no private context; every required decision made or explicitly scoped out; every acceptance criterion checkable by test, command, or concrete manual procedure; needed external access named. Any failure makes the item **HITL**, with the failing predicate as the stated reason | Ready-for-agent (upstream name), Triage (unscoped) |
...
| **House commit style** | `ship`'s default shape for commit and PR prose: plain-imperative subject (~50 chars, hard 72, no type prefix), a body only when the subject can't carry the why, and a PR description that is the commit body plus links. Overridden only by a convention the project *explicitly declares* (a `CLAUDE.md`/`CONTRIBUTING` style block, a commit-lint config, a PR template) — never by undeclared history, which `ship` does not read for style | Fallback style (retired — it was briefly the evidence-gated fallback; now the default), Commit convention (the project's declared one, which still wins) |
