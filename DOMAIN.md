...
| **Behavior contract** | The pre-run specification `black-box-check` validates against — user tasks, expected observable behavior, Anti-cheat probes, required evidence — written *before* touching the target, so it states what the target should do, never what it was observed to do | Test plan (broader), Spec (overloaded) |
| **Anti-cheat probe** | A `black-box-check` move that distinguishes real work from displayed success — varied input the output must follow, persistence across refresh, invalid-input handling, and confirmation that buttons and commands do real work rather than only displaying success text | Smoke test (different purpose), Sanity check (vague) |
| **Negative-space pass** | `review-changes`' closing move on a substantial diff (skipped only when the diff is small or mechanical): one fresh subagent gets the diff and the vetted findings and hunts only where the findings did not go — they map spent attention; an empty return is a good outcome | Blind-spot pass (upstream name), Second pass (vague) |
