...
`scripts/git-hooks/commit-msg` checks a commit message against the deterministic half of the house style in [`src/committing/references/commit-style.md`](src/committing/references/commit-style.md) — subject cap and trailing period, the Conventional Commits prefix this repo does not declare, the blank separator, code fences, `Summary`/`Changes`/`Testing` headings, and the 72-column body wrap; the imperative-opener check warns rather than blocks. It exists because `committing` is model-invoked and so can carry no load gate of its own — the hook is the watcher, and its rejection names the reference so the block is also the load instruction. It checks SHAPE only: register, one-logical-change, and the tell catalog still need a read, so never prune `commit-style.md` against a green run. Inert until `bash scripts/setup-hooks.sh` points `core.hooksPath` at `scripts/git-hooks/`. `bash scripts/commit-msg-selftest.sh` proves its rules still fire — run after changing it.
