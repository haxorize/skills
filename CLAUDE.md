...
`bash scripts/lint-adrs.sh` checks `docs/adr/` against [`adr-format.md`](src/adr/references/adr-format.md)'s two-way rules — unique numbers, supersession links that resolve and link back, the forward pointer on every record an amend mention names (in the link, bold, and unlinked prose forms — its header lists what a pass does not cover), `Revisit when:` lines with text, settled `## Deferred` lines pointing at a dated amendment, corrected `## Consequences` bullets pointing at one, and every `](NNNN-*.md)` cross-reference resolving to a record that exists; `--help` for its exit codes. ADR-0068 records the decision. Run `lint-adrs.sh` after touching a record; run `bash scripts/lint-adrs-selftest.sh` after touching a check — it grades every check both ways against `scripts/lint-fixtures/adr/` and `scripts/lint-fixtures-clean/adr/`, and never reads `docs/adr/`.
