...
`bash scripts/lint-skills.sh` (`--help` for its exit codes) checks the skill tree against the conventions in [`src/write-skill/SKILL.md`](src/write-skill/SKILL.md) — size caps (the 200-line FAIL on SKILL.md and reference files; a byte WARN on SKILL.md alone past the platform's re-attach bound, whose figures the script header dates), frontmatter (description length/colon, a single-line description scalar, the invocation-axis flag, `requires:` resolution), sibling-file byte-identity, the ban on skill bodies citing repo ADRs by number, reference-link resolution (inline `[text](path.md)` links only — the script header names the link forms and resolution cases it does not reach, so a clean run isn't a claim about those), router coverage (every skill mentioned in `which-skill` and in `README.md`), the two-way `requires:` check (a skill the body calls with the Skill tool is declared, and calling the Skill tool with a user-invoked skill fails outright; a declared dep is named in the body), the slash-on-model-invoked check (a `/name` naming a model-invoked skill fails, swept over `src/`, `.claude/skills/`, `DOMAIN.md`, and `README.md` — not `global/rules/` or this file), the `global/rules/` admission check both ways (every rule carries a `Depends:` line naming at least one existing skill, and every named skill cites the rule, in the forms the script header names; the size cap and ADR-number ban sweep `global/rules/` too), and the two selftest-pairing checks: every `global/hooks/<name>.sh` whose header carries `# Install note:` (the marker `install.sh` and `post-merge` derive the hook roster from) has an executable `<name>-selftest.sh`, and so does every `scripts/<name>.sh` that is not itself a selftest, a `*-lib.sh`, or an installer. Run before committing skill changes.

`bash scripts/lint-adrs.sh` checks `docs/adr/` against [`adr-format.md`](src/adr/references/adr-format.md)'s two-way rules — unique numbers, supersession links that resolve and link back, the forward pointer on every record an amend mention names (in the link, bold, and unlinked prose forms — its header lists what a pass does not cover), `Revisit when:` lines with text, settled `## Deferred` lines pointing at a dated amendment; `--help` for its exit codes. ADR-0068 records the decision. Run `lint-adrs.sh` after touching a record; run `bash scripts/lint-adrs-selftest.sh` after touching a check — it grades every check both ways against `scripts/lint-fixtures/adr/` and `scripts/lint-fixtures-clean/adr/`, and never reads `docs/adr/`.
