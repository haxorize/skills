...
- **`onboard-repo`** — Wire a repo for the suite in one sitting: the `Issue tracker:`, `Landing:`, and `## Registry` blocks, loop commands and the `Run:` bring-up line (or `## What the product is here` for an instruction-set repo), convention-skill roles, `## Sibling repos` and `## Review lenses` where the repo has them, and the `DOMAIN.md` / `docs/solutions/` seeds, each written only where nothing exists yet. Prints the hook snippet; never edits `settings.json`.
- **`onboard-me`** — A knowledge-transfer session over an unfamiliar repo: evidence-tagged findings, a KT map of what is still dark, one rung per turn, and each topic worth going deeper on named to the learner to take to `/teach-me` themselves. Writes one file itself, `docs/inward-spec.md`, only when the learner asks for it, and the map lives outside the repo; where the repo has no product description it offers one, and on a yes `product-description` writes a `docs/product-description/` directory.
- **`rebuild-contract`** — The pre-rewrite contract for a system about to be ported or replaced: every observer at its declared boundary, a fidelity per surface, and everything they can see written as rules a reimplementer builds from without opening the source, with the audit trail beside it under `docs/rebuild-contract/`. Reads the repo and writes only that folder, after asking. Declares `product-description` and calls it with `--seed` where the repo has none; the architecture it strips out is `onboard-me`'s KT map, and `onboard-me`'s inward spec (`docs/inward-spec.md`) is the seam it reads for that boundary.
...
- **`from-ticket`** — Cold-start loader. Pulls a published ticket (Task / Story / Bug) back into the conversation, auto-detects type, and loads the right shape — parent context, `DOMAIN.md`, ADRs matched against `## Layers touched`. Refuses Feature/Epic with a redirect. Reads AC IDs by `to-story`'s / `to-feature`'s `references/ac-ids.md` rule. Hands off to `implement` or freeform.
...
- **`upgrade-deps`** — Upgrade dependencies in the safe order — security-flagged first, each major its own step with the changelog read and the suite run between, then the minor/patch batch — with a per-package supply-chain audit (publisher, publish age, provenance, tarball diff, license) before anything touches the lockfile. Declares `feedback-loops` for the suite between steps and `adoption-verdict` for a license change, and hard-gates on the latter. npm, pip/uv, NuGet, and Gradle, whose four missing primitives — no outdated command, no audit command, no lockfile by default, and no registry provenance field — get their own reference.
...
- **`capturing-learnings`** — Owns the per-repo `docs/solutions/` solved-problems store on both sides: captures a Learning doc when its gate holds, or an incident learning when a resolved production incident meets one of its triggers, and serves the symptom-keyed retrieval protocol any skill reads the store with — both kinds carry the `symptoms:` frontmatter the grep lands on.

### Design

- **`codebase-design`** — Shared vocabulary and principles for designing deep modules (module / interface / depth / seam / adapter), the model seam (one contained judgment behind a typed interface; a model's write to user-owned data behind an accept), the interface an agent-driven CLI must carry, and what a change to a published interface owes its consumers (`references/published-interfaces.md`: breaking vs non-breaking, the zero-consumer license, the deprecation questions); declared by `review-architecture` and `review-changes`, and used as vocabulary (no `requires:` edge) by `diagnosing-bugs`, `work-item-shape`, and `ship`'s PR-body reference.
...
- **`tdd`** — Test-driven development using vertical slices. Universal RED/GREEN/refactor core; project commands resolved via CLAUDE.md `## Commands`; stack-specific finalization deferred to convention skills and `feedback-loops`; names ACs by `to-story`'s / `to-feature`'s `references/ac-ids.md` rule.
...
- **`doc-claims`** — Judging a document's claims against what it answers to — the code and tests, the running product, or the sources a derived doc came from — one verdict per claim (PASS/FAIL/UNSUPPORTED/STALE/CONTRADICTS, where STALE covers an evaluation-ledger row past its expiry and CONTRADICTS a pair of in-force instructions that disagree; UNGROUNDED is the derived-documents pass's own), with the source map and the both-directions paraphrase-drift rule for derived documents. Declared by `/verify-docs` and the repo-local `sweep-corpus`.
...
- **`phi-safe-code`** (Domain) — Keeping member and patient data out of every sink it leaks into — logs, error text, URLs, file names, fixtures, analytics, crash reports, model prompts, embeddings, caches, queues, the clipboard, commits, chat — by tracing each field to each sink and allow-listing fields by name; small-cell suppression on rollups, the audit trail, the malformed-input rule, and the BAA gate for external sinks, recordings, images and scans included. Mechanism only: the allowed-field list and retention figures are the project's convention skill.
...
- **`aeo`** (Domain) — Making a public site readable, navigable, and accurately extractable by AI answer engines and agentic browsers: the served DOM as the contract, structured data that declares only what the prose carries, extraction-shaped content, and a crawl and access policy decided per agent class. Rank tracking, keywords, and share-of-voice stay marketing's; the words stay `health-literacy`'s.
...
`.claude/skills/` holds two skills that run only inside this repo and are never hoisted: **`mine-skills`** (the mining-round opener — clone, scan, and inventory the repo sources and the transcript corpus, read under the standing lenses, re-check the prior round's parks, and write the round's ledger rows a grill ratifies) and **`sweep-corpus`** (the scheduled health sweep — lint, the `doc-claims` check over the four documents that claim what the suite is, and the cross-reference and router checks — report-only, writing one dated `docs/health/<date>.md` per run against an additive `docs/health/open-findings.md`, plus the hand close a fix session runs outside a sweep). They live outside `src/` because they name this repo's paths and procedure; `scripts/lint-skills.sh` scans them for the slash-form sweep and the evaluation-ledger vocabulary check only — its frontmatter, size and reference-link passes skip them — and the router does not route to them.
...
- **Learning docs and incident learnings** live in `docs/solutions/` per repo, captured by `capturing-learnings` — a Learning doc (`<slug>.md`) is a solved problem in four sections (Problem / What didn't work / Fix / Prevention); an incident learning is a resolved production incident's review in six (Summary / Timeline / Root cause / Contributing factors / Impact / Action items) at `<yyyy-mm-dd>-<slug>.md`; both carry symptom-keyed frontmatter. The store is created lazily; new captures land flat at the root, and subdirectories from other tooling are tolerated.
...
- **Registry block** — a package-curation policy is declared per-repo in `CLAUDE.md` under `## Registry`, written by `onboard-repo` and read by `upgrade-deps`: `Minimum release age:` (the proxy's floor, in days — it raises `upgrade-deps`' audit floor and can never lower it, since the floor is the maximum of this number, the manager's enforced one, and the skill's 7-day default), `Unattested publishers:` (`accept`, `ask`, or `defer` — what an upgrade does with a package whose target carries no registry attestation and whose other checks are clean; `ask` when the line is absent), and any other curation line. The number and the policy are the org's and live only here.
- **Run line** — the bring-up contract is one `Run:` line under `## Commands` in `CLAUDE.md`, written by `onboard-repo` and read by `product-description`'s verification pass: launch, readiness signal, sibling services and their start order, seed, and the canonical record, in that order. Absent, the verification pass is blocked until it is written.
- **Instruction-product marker** — a repo whose product is an instruction set (skills, rules, prompts) gets `## What the product is here` in `CLAUDE.md` from `onboard-repo` instead of a `Run:` line, so a runtime check knows what "running" means there and skips bring-up.
- **Deferred bumps** live in `docs/deps-deferred.md` per repo, one entry each, opening on a grep-able header line — `<package> <from> → <to>; review by <date>` — with the audit reasoning on continuation lines beneath it and the grouping left to the repo. Written by `upgrade-deps` at close and read back by its next run off the header line; a re-deferred package has its entry replaced rather than a second appended, and a package that lands has its entry struck with the bump. A bump deferred for a missing attestation ends `; no review-by date` in place of the date, since no date retires one: it resurfaces every run until the package is attested or the repo's `Unattested publishers:` value changes.
- **Landing key** — the landing policy is declared per-repo in `CLAUDE.md` under a `Landing:` block, read by `committing` and `ship` before any outward act. Six lines: `Branch policy:` (`trunk` or `branch-per-ticket`, with a naming pattern where the repo has one), `PR required:` (`yes`/`no`), `Push pre-authorized:` (`yes`/`no`), `Ticket close pre-authorized:` (`yes`/`no`), `Review required:` (a bare `yes`/`no`, with no parenthetical, because the hook's pattern anchors at end of line; absent means `no`, except in a repo `scripts/lint-skills.sh` gates, where a block that exists must name the key so a deleted gate cannot read as a gate never configured — `yes` gates the push through the `review-receipt` hook on a report stamped with the pushed tree), and `Defect policy:` (default `fix, don't file` — defects found mid-work are fixed in place or parked, and filed as tickets only by `to-bug` on the user's ask). An act the block pre-authorizes proceeds on the ask that started the work; every other outward act asks first. No block means nothing is pre-authorized. The two middle keys' legacy `pre-authorised` spelling and the rename that retired it are `committing`'s rule.
- **Sibling repos** declared in `CLAUDE.md` under `## Sibling repos` — one line per sibling, `` `<relative-path>`: <relationship> `` — written by `onboard-repo` so `to-tasks` can flag cross-repo blockers and `from-ticket` can point a wrong-repo session at it by name. A repo that declares none gets vanilla behavior.
- **Review lenses** — a repo may declare extra or specialized review lenses in `CLAUDE.md` under `## Review lenses`, written by `onboard-repo` and read by `review-changes` on top of its standing set (this repo's block adds the pruning test to the instruction-file lens). The block adds lenses or narrows an existing one to the repo's subject; it never changes the review's contract, and a repo without one gets the standing lenses.
