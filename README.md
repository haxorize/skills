...
- **Registry block** — a package-curation policy is declared per-repo in `CLAUDE.md` under `## Registry`, written by `onboard-repo` and read by `upgrade-deps`: `Minimum release age:` (the proxy's floor, in days), `Unattested publishers:` (`accept`, `ask`, or `defer` — what an upgrade does with a package whose target carries no registry attestation and whose other checks are clean; `ask` when the line is absent), and any other curation line. The number and the policy are the org's and live only here.
...
- **Deferred bumps** live in `docs/deps-deferred.md` per repo, one line each — `<package> <from> → <to>: <reason>; review by <date>` — written by `upgrade-deps` at close and read back by its next run. A bump deferred for a missing attestation ends `; no review-by date` in place of the date, since no date retires one: it resurfaces every run until the package is attested or the repo's `Unattested publishers:` value changes.
