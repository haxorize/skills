...
- **`accessible-ui`** (Domain) — Building UI a keyboard and assistive technology can operate, and claiming no more than the evidence supports: state in the accessibility tree, a label that is not a placeholder, a live region that exists before its text, a dialog that traps and restores focus, and a per-change criterion ledger where every WCAG 2.2 criterion touched ends in one of five states with its evidence, AAA never presented as AA; the widget contracts run from the toggle chip to data tables and charts, and the native lines, the motion and timing rules, and the scaling and reflow thresholds each sit behind a condition the body names. Mechanism only: the scanner, CI step, component library, and target level are the project's convention skill.
