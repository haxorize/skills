...
3. **Recurrence-plausible** — the class of problem can bite again in this repo or its siblings: the pattern is used repeatedly, or the trigger is easy to re-create. Its sharper form is a deletion test on the store: if this doc were deleted, would a future run be *steered differently*, or would only the history be lost? An account of what happened that changes nothing a later session does belongs in the commit log — the store exists to change behavior, not to remember.
