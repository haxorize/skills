...
1. **Verified** — the fix is in and the original symptom is confirmed gone (the reproduction loop re-ran green). Never capture an unverified theory. A results writeup offered as a learning — a shipped feature's numbers — is not one; capture only the learning behind a number that surprised, and only when the effect holds on the whole population with its confounders named, not on a segment chosen after the fact.
