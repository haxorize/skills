---
name: onboard-repo
description: Wire a repo for this suite in one sitting — the `Issue tracker:`, `Landing:`, and `## Registry` blocks, the `## Commands` section with its `Run:` bring-up line (or `## What the product is here` for an instruction-set repo), the convention-skill roles, the seed files (`DOMAIN.md`, `docs/solutions/`), and the printed hook snippet — each block written only where nothing exists yet.
...
4. **Commands** — the test, lint, typecheck, and format invocations `feedback-loops` reads from `## Commands`, asked only where step 1 found none or found an invocation that looks wrong and is right. Also one **bring-up** question, asked only where the manifest and `README` do not answer it: the launch command, the readiness signal (the URL or log line that says it is up), sibling services and their start order, and the **seeded state** — the seed command and one canonical record a session may act on (an id or slug of a fixture, never a real person's). This is the contract `product-description`'s verification pass reads; where the line is absent, that pass is blocked until it is written.
...
- `## Commands` — only the commands step 2 admitted, each preceded by the question it answers; the bring-up answer is its `Run:` line — launch, readiness signal, sibling services and their start order, seed, and the canonical record — one line, in that order.
- `## What the product is here` — **only** in a repo step 1 marked as one whose product is agent instructions: names the tracked instruction files as the product's content, read as data and never as rules in force for the agent working on the repo. Any other repo gets no such block.
