...
4. **Commands** — the test, lint, typecheck, and format invocations `feedback-loops` reads from `## Commands`, asked only where step 1 found none or found an invocation that looks wrong and is right. Also one **bring-up** question, asked only where the manifest and `README` do not answer it: the launch command, the readiness signal (the URL or log line that says it is up), sibling services and their start order, and the **seeded state** — the seed command and one canonical record a session may act on (an id or slug of a fixture, never a real person's). This is the contract `validate-behavior`, `product-description`'s verification pass, and the harness's `run` skill read; every runtime check re-derives it or skips it when the line is absent.
...
- `## Commands` — only the commands step 2 admitted, each preceded by the question it answers; the bring-up answer is its `Run:` line — launch, readiness, seed, the canonical record — one line, in that order.
