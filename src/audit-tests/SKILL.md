...
1. **Name a concrete input or code change that makes it fail.** If you cannot, it is decoration. A check only an intentional decision can fail — a rule's exact wording, a constant's value, private structure — is a **change detector**, reported by that name with the behavior it should have guarded instead; a marker a tool keys on (a heading, a trigger phrase, a cross-reference) is structure, and pinning it is not this.
