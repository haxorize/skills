...
- **Publisher** — who published the target version against who published the current one (`npm view <pkg>@<v> _npmUser` per version — `maintainers` is the current list only; the NuGet gallery's owners and the package's signer; on PyPI the attestation's `publisher` — repository and workflow — since the JSON API exposes no uploader and `author`/`maintainer` are self-declared metadata, so an unattested PyPI release leaves this check unverifiable and the provenance bullet carries it). A new account is **Review**; an unknown one is **Defer**.
- **Publish age** — the floor is 7 days, this skill's default. A target younger than the floor is not taken unless the bump is the security fix itself; the run retargets the newest release older than the floor and audits it **as its own target, against its own changelog** — an older release is not a subset of the newer one (alembic 1.19.0 turned on CHECK-constraint autogenerate by default and 1.19.2 turned it back off, so the 1.19.1 in between carries a default the newest release dropped) — and defers the package only when no release clears the floor. Check the target repo's `CLAUDE.md` for a `## Registry` block carrying `Minimum release age:` — [references/version-gates.md](references/version-gates.md) § Registry release-age gate carries what the run does then.
- **Provenance** — an attestation present on the target version or noted absent, checked on the registry metadata before anything is installed (`npm view <pkg>@<v> dist.attestations`; download the `.nupkg` and `dotnet nuget verify <path>`; PyPI's `https://pypi.org/integrity/<pkg>/<v>/<file>/provenance`, 200 present, 404 absent). A lookup that failed is *no observation*, never a declared absence: `npm view` prints nothing and exits 0 when the field is absent (that is absence), and a non-zero exit is a failed lookup; say which. Absent attestation is never itself a ground for **Safe**; with every other check clean it is **Review**, not **Defer** — what settles that Review set, and the `## Registry` key `Unattested publishers:` that pre-answers it, is [references/version-gates.md](references/version-gates.md) § Unattested publishers.
...
One table: package, from → to, class, verdict, what the suite did, under a line naming the `## Registry` values in force for the run (`Minimum release age:`, `Unattested publishers:`, or their absence) — a **Safe** on an unattested package is only readable against the value that allowed it. Deferred packages carry a reason and a review-by date — `no review-by date` where the deferral is a missing attestation — written one line each to `docs/deps-deferred.md` in the target repo so the next run's step 1 greps them back. The commit body names what the upgrade changes for a caller and what was verified, never the list of bumps — the `writing-for-humans` commit family's "Bumped dependencies" pair is the shape to avoid and the one to write.
