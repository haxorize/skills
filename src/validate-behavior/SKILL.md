...
2. **Prepare runtime access** — target URL, CLI command, API endpoint, fixture data, artifact path. Credentials come through the environment or the user's secret tooling; never copy a credential value into notes, output, or evidence. Start what you drive under your own control, or have the user or a context-free session start it for this check, and before the first probe confirm the port answers that process (`lsof -ti :<port>` for the PID, then `ps -o user=,args= -p <pid>` for its owner and command line). An empty `lsof` is not an empty port: `lsof` omits sockets other users own, so probe the port with a connection attempt as well. A listener that answers but `lsof` cannot see, or that nobody started for this check, is a foreign instance: ask whose it is before the first probe, and where it is the user's own it is the shared instance refused below. Tear down only what you started, by the handle you hold — the PID, the container, the session — never by process name: `pkill node` takes the user's editor server with it.
...
- **UNVERIFIABLE** — required runtime access, credentials, fixtures, or tools are missing (including blocked on source-blindness or on a shared instance) — nothing could be observed either way. A truncated or unreadable observation is UNVERIFIABLE for its clause, never a pass. A stand-in — a mock, a stub, a fixture service — counts only where it replaces a dependency the production path itself calls out to; one that skips the renderer, an RPC, persistence, or the tool path under test makes the clause UNVERIFIABLE, and the report names the dependency the stand-in replaced.
...
A contract clause no visible surface can reach is fixed by a user-facing affordance — a command, a flag, a page — never by a private test API; the absence is itself a finding, reported as FAIL on that clause. The two verdicts do not overlap: a clause with no visible surface at all is FAIL; a clause whose visible surface is driven through a stand-in that skips a production dependency is UNVERIFIABLE.
