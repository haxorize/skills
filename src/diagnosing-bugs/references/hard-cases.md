...
When the failure arrives from CI rather than a local run, read the run itself before anything else — a canceled or superseded run is not a red, a skipped job is not a green, and a rerun that passes is evidence of nothing but non-determinism — it names no cause, and a resource limit or retry policy is not changed on its strength; on recurrence keep the run id and obtain the runner's diagnostics first; a red is reproduced under the failed job's own configuration (its matrix entry, runner, flags), and a green run of the broad local suite does not clear it. Then the spread shape decides the investigation before any culprit hunt. One branch only, and not a merge-queue gate branch → suspect that branch's change (the normal loop). Many unrelated branches in a tight window → the trunk is broken (PR CI runs merged with the trunk): walk the trunk's green-to-red boundary for the culprit, and sanity-check that the suspect commit plausibly touches the failing area before naming it; a walk that finds no plausible commit points outside the repo — a dependency release, a runner-image change, an expired credential — so check what changed in the environment over the same window. Sporadic across weeks → a flake; treat it as a non-deterministic bug (§ Non-deterministic bugs, entered through Phase 1). Red on a merge-queue gate branch only → the commit already passed the PR's own CI, so the failure is a conflict with what landed or queued in between, or a queue-mate's break: diff the gate branch against trunk before hunting in the PR. When the asker's change is not at fault, say so explicitly — that is usually the single most valuable sentence in the answer.

## Non-deterministic bugs

The goal is not a clean repro but a **higher reproduction rate**. Loop the trigger 100×, parallelize, add stress, narrow timing windows, inject sleeps. A 50%-flake bug is debuggable; 1% is not — keep raising the rate until it's debuggable. When a flake tracks test *order* rather than timing, suspect leaked state: run the failing case in isolation and again after the full suite, and check singletons, shared caches, and leftover fixtures.

Five moves look like flake fixes and are not, when reached for to make the red go away: raising a timeout past the race it lost, adding a retry, serializing the suite, skipping the test, and weakening the assertion until it stops disagreeing. Raising a timeout genuinely calibrated below the work it waits on is a real fix — the difference is whether you can name the duration it should have allowed. Each buys a green run by deleting the signal, and the non-determinism stays in the product, where a user meets it instead. A skip that has to stay is a quarantine, and a quarantine names its ticket and its expiry — a skip with neither is the false fix above. Reproduce it smallest, fix the source, then **soak** the repaired test — re-run it many times under the conditions that used to break it (same parallelism, same ordering, same clock pressure), because one green run says nothing about a failure that appeared once in twenty. And the run being read is chosen, never defaulted: where the runner retries, the record it hands back is the *last* attempt, which on a flake is the green one, so the failed attempt is selected by its attempt number before any log, screenshot, or replay is opened — or the diagnosis starts from a run in which nothing went wrong.
