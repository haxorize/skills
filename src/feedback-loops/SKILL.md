...
This pass is **mechanical only**: it runs the checks and never judges whether code is *good*.
...
This pass runs the checks; it never grades the code. Simplification lives in the build and refactor beat that precedes it, and quality, conformance, and correctness findings are `review-changes`', which reads the diff this pass leaves clean. A check that is red because the change is wrong is a defect to fix, not a loop to run again — the diagnosis is `diagnosing-bugs`'. Landing what this pass finished — the commit, the push, the ticket — is `committing`'s; the close names the commit as the next act and performs none of it.
