...
# Color only on a terminal that has it, and never when NO_COLOR is set (the
# no-color.org convention every color-emitting helper below inherits).
...
# in color, the color closed before the human types, then one line read into
