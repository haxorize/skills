...
| **Operations** | The healthy end state, the observation window, the rollback trigger — the three lines `ship`'s post-deploy watch reads off the item before the deploy. The healthy end state is the production signal § Acceptance criteria asks for, and the window is that signal's baseline; an ops item owes the three lines, not a fourth criterion |
