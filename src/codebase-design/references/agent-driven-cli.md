...
- **A machine shape on request.** `--json` (or an equivalent) on every command that returns data, with a stable field set; prose output is for the person, and an agent parsing prose breaks on the first wording change. Stable means every response carries every key of the shape, the clean path included — `"truncated": false, "hint": null`, never the key omitted — because an absent key is the one thing a model reading the output fills in with invention.
