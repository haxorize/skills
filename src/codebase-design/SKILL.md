...
- **The interface is the test surface.** Callers and the module's behavioural tests cross the same external seam; internal seams serve only the module's private tests. If verifying the module's *behaviour* requires reaching past the interface, the module is probably the wrong shape.
