...
- `<dialog>` opened with `showModal()` gives the trap, `Escape`, and inertness for free; a hand-built modal sets `role="dialog"`, `aria-modal="true"`, `inert` on the page root's other children, and traps Tab itself — `inert` and not `aria-hidden`, which hides the rest from the screen reader and leaves it focusable, so Tab walks into content the reader cannot announce.
...
## Option groups and form errors (1.3.1, 3.3.1, 3.3.2, 3.3.3, 3.2.2, 1.3.5, 3.3.7, 3.3.8)
...
## Binding submission (3.3.4)

A submission that deletes data, commits money, or binds the person legally is reversible, confirmed, or reviewable before it commits (3.3.4 Error Prevention).

- The confirmation is where the destructive action becomes the primary control; the review step shows what was entered, editable.
- Run: submit the binding action; a confirmation, review, or undo stands between the press and the commitment.
