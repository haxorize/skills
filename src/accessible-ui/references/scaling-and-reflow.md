...
- The root font size is not locked in pixels, so the browser's own text-size setting still reaches the page. The text-size setting is not zoom, and the check runs on the setting: set the browser's default font to 32px and confirm every control still contains its label — a px-everything page passes 200% zoom and still clips this user.
- Anything holding text gets `min-height`, never `height`; media queries are written in `em`.
