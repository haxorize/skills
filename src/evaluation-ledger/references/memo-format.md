...
Where the memo decides adopt-or-not, section 1 is the `adoption-verdict` schema per candidate or for the comparison — grade, incumbent, verified facts, conversation hypotheses, conditions, reversal trigger — with the ledger's `verified` rows as the verified facts and its `marketed` rows as the hypotheses. A watch has no grade: section 1 is each obligation with its date and its row.
