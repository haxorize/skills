...
- **Completion criterion** — the condition that tells the agent a unit of work is done. Three properties make it a lever. *Clarity* (can the agent tell done from not-done?) resists premature completion. *Demand* (how much it requires — "every modified model accounted for," not "produce a change list") drives thorough legwork, and binds flat reference too ("every rule applied"), which is how a skill with no steps still carries an exhaustiveness bar. *Non-answers* (what does *not* count — partial progress, a special case, a reduction to another unproved statement, a bounded check standing in for the full one, a best-effort summary) block the answer-shaped near miss: under pressure to finish, a model produces something finished-looking, and only an enumerated non-answer tells it that this is not it. The strongest criteria are checkable, exhaustive, and name their near misses. *Non-answers* guards *done*; the **exclusion spec** in the form table below guards *shape* — what a bounded output may not contain — and a criterion names its near misses whether or not the output also carries one.
