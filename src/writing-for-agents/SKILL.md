...
- **Unobserved pitfalls** — a pitfall or warning earns its line only on *observed* evidence: an agent actually made the mistake, or a human caught it in review. A repo scan yields hundreds of trap-looking facts and none of them predict real mistakes — a surprising scan finding is a question to ask, not a line to write. The retire direction inverts: "nothing has failed lately" is not evidence a rule is dead — a working rule erases its own evidence; retire on a control run (the No-op diagnostic), never on quiet.
...
- **Quote the canon, don't paraphrase it** — when a brief or prompt applies a rule another document owns, carry the owning line verbatim; a paraphrase drifts, and every agent briefed with it inherits the drift.
