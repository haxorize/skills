...
- **Unobserved pitfalls** — a pitfall or warning earns its line only on *observed* evidence: an agent actually made the mistake — here, or in the upstream a mined rule came from — or a human caught it in review. A trap-looking fact turned up by a repo scan is not that evidence: a surprising scan finding is a question to ask, not a line to write. The retire direction inverts: "nothing has failed lately" is not evidence a rule is dead — a working rule erases its own evidence; retire on a control run (the pressure scenario rerun without the line), never on quiet.
