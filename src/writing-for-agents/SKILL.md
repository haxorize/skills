...
description: Writing conventions for any document an agent consumes — a skill body, CLAUDE.md/AGENTS.md, a reference file, agent-read DOMAIN.md or ADR prose. Use when writing or editing such a document, when deciding what to inline versus disclose behind a pointer, when pruning an instruction file, or when a doc restates what the environment already answers.
---

# Writing for Agents

A document an agent consumes exists to wrangle determinism out of a stochastic system. The goal is **predictability** — the agent taking the same *process* every run, not producing the same output. The packaging differs — a skill, a `CLAUDE.md`, a reference reached by a pointer — the writing does not. The deep vocabulary (leading words, context pointers, completion criteria, branches), the failure-mode taxonomy, and the form-to-failure wording table live in [references/predictability.md](references/predictability.md).
