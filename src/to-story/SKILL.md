...
On ADO, in no-repo CLI-only mode, save the resolved Feature ID to the same memory entry `tracker-resolution.md` keys by tracker context, so the next invocation does not re-look it up.
