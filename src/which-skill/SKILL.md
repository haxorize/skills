...
- **`/address-findings`** — act on a `/review-changes` report in one pass: fix what is mechanical, batch the rest into one question with recommendations, close with a disposition per finding ID (FIXED / DECLINED / DEFERRED / ABANDON), and stop. No argument picks the newest report for this repo. Re-review is your call; it never loops. The judgment per finding inside the pass — and the same one pass when a reviewer's comments land on *your* PR — is the `receiving-review` behavior: verify each claim before implementing it, every thread answered once the fix is pushed.
