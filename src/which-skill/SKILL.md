...
Under all of them sits the always-on **rules layer** (`~/.claude/rules/`) — evidence beside the claim, no unasked outward acts, the three-bin ask gate, the truncated-artifact invariant, the outbound dash sweep. Nothing routes to it; it binds every flow below with no skill loaded. Operating safely has no skill of its own — three homes: a destructive command is checked before it runs by the harness's own reminder and `committing`'s dirty-path rule, which also owns a working tree another session shares; a leaked secret is `committing`'s rotate-first paragraph and `scripts/security.sh`.
