...
`product-description` records the outside for a person, per feature area; this contract records every observer at the boundary for a reimplementer, and the description is one of its inputs. `onboard-me`'s KT map records the inside for a person learning it — what is lit and what is still dark — so architecture stripped out here has somewhere to go, though that session maps what its learner needed and never claims to be exhaustive; on request it also writes the **inward spec** — architecture, contracts, data model, for a reader who will open the source — which is this contract's inward sibling and never a substitute for it. `validate-behavior`'s behavior contract is **one** contract for **one** change, checked against a running target by someone who did not write it; this one covers every observer at a boundary, is derived from the source on purpose, and is written for someone who will never see that source. Whether a contract that already exists still holds is `doc-claims`'; what the rebuild should do *differently* is a work item, shaped by `work-item-shape`, and never a line in this document.
