...
| **On-call shift note (the outgoing on-call, per shift)** | Five sections, each present or an explicit "none": active incidents, ongoing investigations, recent changes, known issues with their workarounds, upcoming events; a section left blank is an incomplete note, and an incomplete note is a postmortem action item |
