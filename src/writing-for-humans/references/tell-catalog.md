...
- **Em-dash habit** — budgeted, not banned: none in short copy, one or two in a long draft where they clearly beat commas or parentheses; delete clusters; the outbound row bans them outright, and the global `outbound-dash-sweep` rule enforces it. *Displacement partner chain:* em dash → semicolon → colon — the punchy appended clause survives the ban by swapping its glyph; fix the clause, not the mark. **A fixed field separator is not a cluster:** in a labeled record — the **Blockquote stanza**'s `**Label** — content` lines — the dash does a table pipe's job, one per line by construction, and the count says nothing about prose habit. The ban reads appended clauses, so it applies to the content after the separator and never to the separators themselves.
