...
**Random stimulus.** Force a connection to a concept with no relation to the problem — "how is [problem] like [random]?" — the forced-feel is the point; it routes around the habituated pathway. You picking your own "random" word is not random: the same fixated attention that locked the frame will pick a word adjacent to it. Source externally — ask the user for a word, fetch a random article, or hash-pick from a fixed deck; commit to the first draw, re-rolling defeats the method. *Fired if* the connection genuinely resists at first and working through it produces an angle outside the prior search space; a word that feels immediately "relevant" came from attention — draw again properly. Where the near-duplicates come from a model — this one included — a second technique, **verbalized sampling**: ask for the candidates each with the probability it would be the first answer, load the problem's real context first (a thin prompt yields a generic tail), and take the tail under a tenth rather than the top. *Fired if* the stated probabilities span an order of magnitude top to bottom; a flat spread is the mode renamed.
