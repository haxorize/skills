...
| **Generator keeps returning near-duplicates** | Random stimulus, then verbalized sampling |
...
**Random stimulus.** Force a connection to a concept with no relation to the problem — "how is [problem] like [random]?" — the forced-feel is the point; it routes around the habituated pathway. You picking your own "random" word is not random: the same fixated attention that locked the frame will pick a word adjacent to it. Source externally — ask the user for a word, fetch a random article, or hash-pick from a fixed deck; commit to the first draw, re-rolling defeats the method. *Fired if* the connection genuinely resists at first and working through it produces an angle outside the prior search space; a word that feels immediately "relevant" came from attention — draw again properly. Where the near-duplicates come from a model — this one included — a second technique, **verbalized sampling**, is the fallback when no external source is reachable, and its output is a candidate list, not a pick: ask for the candidates each with the probability it would be the first answer, load the problem's real context first (a thin prompt yields a generic tail), then hand the list to the external draw above — the user, the deck — to choose from. *Fired if* the tail sits under a tenth of the top candidate's probability; where the top is itself under a tenth, the whole set is the tail. The probabilities are self-reported, so the gate grades spread, never truth; a flat spread is the mode renamed.

**SCAMPER.** For iterating on an existing artifact — walk all seven transforms once, no favorites in advance: **S**ubstitute, **C**ombine, **A**dapt, **M**odify/magnify/minify, **P**ut to another use, **E**liminate, **R**everse/rearrange. *Fired if* at least one transform produced a candidate you wouldn't reach by asking "what's a better version of this?"; seven adjacent polishes mean the artifact is the wrong unit — zoom out and Reframe.
