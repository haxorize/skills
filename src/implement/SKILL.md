...
3. **Declare the edit boundary.** Name the narrowest directory in each layer the slice touches that contains its files there — `src/api/scores/`, `src/web/scores/`, `migrations/`, never the root that happens to contain them all — and say it before the first edit. Name, in the same breath, any `DOMAIN.md` entry whose ground the slice touches, and the ADRs `from-ticket` § ADR match against `## Layers touched` already surfaced: a standing record is consumed, never re-decided mid-slice. A fix that needs a file past that boundary stops and asks — **Proceed** (widen it, with the reason), **Split** (the outside part becomes its own slice), or **Rethink** (the plan was wrong) — never widens quietly. `diagnosing-bugs` inherits this boundary when it runs under `implement`, and declares its own once its hypotheses are ranked otherwise.
4. **Capture the before, where the slice changes what renders.** Take it now, before the first edit, at the viewport and crop the after will use — with the driver the project's convention skill names, or ask the user for the shot when none exists — because one rebuilt later is slow and wrong; the PR body carries both (`ship`'s `references/pr-body.md`).
...
- **Non-testable:** build the change directly, then do a cleanup pass — `/simplify` over what you wrote, applying what it finds, and the naming discipline over anything the slice named, renamed, or moved. Call the Skill tool with `discoverable-code` there. This is the direct path's refactor beat; on the testable path `tdd` runs both beats itself.
