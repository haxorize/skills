...
- **Extract at the third caller, or when the concept has a domain name.** Two call sites are duplication you can see; a helper, abstraction, or option nobody asked for needs its caller named before it exists. This is the **third-caller clause**; the refactor beat on both paths runs under it, and `tdd`'s refactor step carries it for the testable path.
