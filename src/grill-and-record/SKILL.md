...
Two rules from the standalone path apply inline too. Before writing, check for an **owning record** — the amend-or-write-new rule in the format doc decides whether this is a new file or a dated amendment, and the search runs before the write, not after a duplicate exists. And the ADR's rationale prose follows the `/writing-for-humans` behavior — load it at the first write if it isn't already live; it stays live for the DOMAIN.md updates too.
