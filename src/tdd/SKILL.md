---
name: tdd
description: Test-driven development workflow using vertical slices. Use when implementing a feature or a story or task already in context, or the user mentions "TDD" or "test first". Also when tests are asked for on code already written, when a slice touches untested code, and when judging whether a test would still pass if the behavior it covers were broken. A ticket still on the tracker loads through `/from-ticket` first.
...
One litmus covers the shapes that check misses: would the test still pass if every function it imports returned nothing? A yes names one of six — five are `audit-tests`' catalog (no assertion, a mock- or absence-only assertion, the tautology above, a pinned constant, a fixture asserted back at itself), and the sixth is a double standing in for the very boundary the test claims to cover; rewrite the assertion or delete the test. And the check has nothing to mutate against in code that arrived with no tests: a slice that changes such code writes a **pinning test** first — named as a pinning test in the test name, which exempts it from the Change-detector ban; it pins what the code does now, right or wrong — so the change has a red to watch.
