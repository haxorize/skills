...
- **Conditional — document- and handoff-gated:** a diff that touches a document, a skill, or an instruction file — a `SKILL.md`, a `references/*.md`, `CLAUDE.md`, a rule under `global/rules/` or `~/.claude/rules/`, or a hook — or a handoff input — opens [references/lens-briefs-docs.md](references/lens-briefs-docs.md): each lens's trigger and brief.
...
**Write the report to the landing zone** as well as presenting it, as the `<repo>-<date>-<slug>.review.md` file `handoff`'s "Where to write it" section defines, so `/address-findings` with no argument and the `review-receipt` hook both find it, and the close names the report's **absolute, resolved path** — never `$TMPDIR/…` or a bare filename (the receipt contract, and where the report may not be written, is [references/tree-stamp.md](references/tree-stamp.md)). The global evidence rule (`~/.claude/rules/evidence.md`) governs every count in the report — finding totals, files covered, commits since — re-measured at write time.
