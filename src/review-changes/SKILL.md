...
Beside the Callouts sits the **coverage ledger**: every file the diff touches, accounted for — reviewed, or skipped with a stated reason (generated content, vendored code, a mechanical rename). A fan-out that under-read a large diff returns a report indistinguishable from full coverage; the ledger is where that silence becomes visible. When every file was reviewed, it collapses to one line.
