...
**A report written into the tree it names spells that hash `git rev-parse HEAD^{tree}`.** A review report lives in the landing zone, outside the repo; a `sweep-corpus` health report lives at `docs/health/<date>.md`, inside it. The one-liner would fold the half-written report into its own hash and name a state that exists for one instant, so the producer runs on a clean working tree and writes `HEAD^{tree}` instead. This is a spelling, not a second rule: on a clean tree the one-liner returns exactly `HEAD^{tree}`, so the two agree at the moment of measurement and `HEAD^{tree}` is the one that stays re-derivable after the report lands. A producer that cannot start clean has no stamp to write and says so.

**Committed work is stamped `git rev-parse <target>^{tree}` instead** — a landed commit, or a change whose edits were already committed when the review read them. The one-liner is wrong there: it folds in uncommitted dirt the review never saw, so it can never equal the reviewed commit's tree. Read the report's header for which target it names before comparing, or a stamp taken the other way reads as a changed tree where nothing changed.
