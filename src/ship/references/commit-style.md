...
- **A large PR may earn a suggested review order — by concern, never by file.** When the diff is big enough that a reviewer would otherwise walk it alphabetically, add a short ordered list of *concerns* — each a cohesive design intent with its `path:line` entry points and a sub-15-word framing — sequenced so the reader never meets a reference before the thing it references. One file may appear under several concerns. More than about 7 concerns is a scope signal, not a formatting problem.
