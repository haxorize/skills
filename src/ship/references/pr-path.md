...
- **Approval is someone else's act.** Open the PR, set the reviewers (from `.github/CODEOWNERS` where the repo has one, else from `CLAUDE.md` where the project declares them; ask when neither does, and never guess a name — on ADO, marking them *required* is a second call, in [ado.md](ado.md)), and **stop there**. Don't wait, poll, or nudge. Report the PR as open and awaiting approval, because that's what it is.
