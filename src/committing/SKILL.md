...
A comment or issue that lands under a bot or app identity — the `gh`/`az` call is authenticated as an app or service principal, or the tracker shows a bot as the author — opens with one line saying it was AI-generated; one that lands under the human's own identity (`gh auth status` / `az account show` names the person) carries no such line, because the account already says who is speaking. The gate above decides whether the post happens, this decides what it discloses. A message drafted *as* the user is `writing-for-humans`' outbound convention, not this.
...
**A credential in the diff or the history is rotated first; the removal is not the fix.** When a credential value appears in the diff (by your read or a scanner's), deleting the line un-leaks nothing — it is in every clone and reflog — and a dead key still fails a scan. Revoking or rotating it is the human's act: ask for it with the ask block before the commit, and the commit does not land until they confirm it happened; then remove the line, and the message says both.
