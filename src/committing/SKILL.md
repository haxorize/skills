...
**A credential in the diff or the history is rotated first; the removal is not the fix.** When the secret scan fires or a key turns up in the change, deleting the line un-leaks nothing — it is in every clone and reflog — and a dead key still fails the scan. Revoke or rotate it, then remove it, and the message says both.
...
| **"works" / "done" for a change whose effect is live — a tracker write, a deploy, an outbound call** | the live path exercised in this session, or the claim carries `UNVERIFIED: live path`; a unit or fixture run is evidence for the code, never for the effect it was standing in for. For a deployed change the marker is lifted by the post-deploy watch's verdict, quoted with its window (`/ship`'s `references/after-landing.md`) |
...
In a repo whose round plan names a deferrals register (the plan's §9), step 5 is preceded by one grep of that register for a row dated today; none found means stop and write one — "none" is an allowed row, and is itself the evidence. A repo without such a plan skips this step.
