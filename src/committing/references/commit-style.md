...
Open this only when drafting a commit message, a PR body, or a closing comment.
...
A routine PR description is the commit body, then the ticket and issue references at the bottom. It opens with the umbrella outcome of the full range `git log <base>..HEAD` — what the change as a whole makes true — never with the bug report that started it or a list of files. Structure (a short list of behaviors, a design note) is earned only by a change genuinely too complex to review without it — the tell catalog's commit-and-PR family names the padding shapes to leave out.

- **The repo's PR template always wins over this shape.** Adapt the final body to the template: keep its headings, checklists, and prompts, answer `N/A` where a section does not apply, and never delete a section because this default is shorter. A box that cannot be ticked is left unticked with the reason on the same line, never ticked with a caveat trailing it. The template's machine-read markers — linking keys, changelog blocks — stay exactly where the template puts them, HTML comments included, since whether a marker is read there is the template's convention and not yours to re-derive.
...
- **The body answers four questions in order** — what changed, what was checked and how, the risks, what had to be fixed along the way — plus what did not change, any published-interface delta, and a before-and-after capture for a change to what renders: `ship`'s `references/pr-body.md` owns those, and this file owns the shape they take.
- **A PR that earns structure may also earn a suggested review order — by concern, never by file.** When the change is too complex to review without a map, add a short ordered list of *concerns* — each a cohesive design intent with its `path:line` entry points and a sub-15-word framing — sequenced so the reader never meets a reference before the thing it references, written at open and not maintained across pushes. One file may appear under several concerns. More than 7 concerns is a scope signal, not a formatting problem.
