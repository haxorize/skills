...
- [ ] The body runs the spine it declares — Workflow skeleton, Principle skeleton (closing on `## Boundary`), or Session controller, per `DOMAIN.md`'s three registered spines; a hybrid discipline carrying a real workflow (`tdd`, `feedback-loops`, `resolving-merge-conflicts`, `adr`, `adoption-verdict`) keeps `## Workflow` and still closes on `## Boundary`. A principle section is headed by the rule's own leading words, so a short noun heading and a full imperative clause both conform and only a topic label fails. A Workflow skeleton carries its mode sections between `## Workflow` and `## Notes` (the `to-*` publishers' update and maintenance modes), and a body with nothing to note ends at its last step rather than opening an empty `## Notes`. A router (`which-skill`) and a body that is one short prose run (`explain`) take no spine, and a file under `global/rules/` takes none of the three — its H1 is a sentence-case proposition, not a display name
