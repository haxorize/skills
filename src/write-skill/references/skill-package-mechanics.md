...
When two skills need the same *inert* doc (a format, a template), don't reach for a repo-root shared folder or a symlink between skills — skills install individually, so anything outside a skill's own `references/` doesn't travel with it. Duplicate the file byte-identically into each skill's `references/`, and register the group in `scripts/lint-skills.sh` (`sibling_groups`) so a copy that diverges fails lint. The duplication tax only stays bounded for short, stable docs. (Reusable *discipline* is the other case — that becomes a model-invoked skill reached via `requires:`, not a duplicated file.)
