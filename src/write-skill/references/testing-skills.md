...
3. **Write the minimal wording**, picking the form from the form-to-failure table in `writing-for-agents`' `predictability.md` reference — open that file: loading the skill loads its `SKILL.md`, and the table is a level down from there.
...
When the micro-test runs against a skill that already ships, two extra rules keep the revision honest: the rep set **must include the failure that prompted the revision** and stay fixed across it — swapping scenarios mid-revision makes before/after incomparable — and **cap the edits per revision at three distinct changes**, fewer as the skill matures: a sprawling rewrite that regresses leaves you unable to attribute the regression to a cause.
