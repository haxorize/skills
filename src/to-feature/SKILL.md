...
In no-repo CLI-only mode, save the resolved Epic ID to the same memory entry `tracker-resolution.md` keys by tracker context, so the next invocation does not re-look it up.
