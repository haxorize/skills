...
The relation is additive, not a replacement: where the calling skill's template carries a `Parent: #N` body line (the `to-*` family's does — it's what `from-ticket` reads to resolve the parent on cold-start), that line stays.
