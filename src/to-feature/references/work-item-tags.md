...
Tagging is best-effort, stamped once at creation. If the create fails because the org denies tag creation (a derived tag doesn't exist yet and permissions block defining it), retry the create without the `System.Tags` pair and give the user the tag list to apply manually. Later drift from a title rename is accepted and not reconciled.
