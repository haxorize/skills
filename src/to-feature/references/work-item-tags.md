...
Tagging is best-effort, stamped once at creation. If the create fails because the org denies tag creation (a derived tag doesn't exist yet and permissions block defining it), retry the create without the `System.Tags` pair and give the user the tag list with the manual command to apply it: `az boards work-item update --id <id> --fields "System.Tags=<tag1>; <tag2>"`. Later naming drift from a title rename is accepted and not reconciled.
