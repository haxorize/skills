...
No create call goes out until all four hold:
...
A rich-text body travels as a file path with the CLI's `@` prefix (`--description @<file>`; `<Field>=@<file>` inside `--fields`), so the content never crosses the shell. The CLI passes a path it cannot open through as the literal `@…` with exit 0, so `test -s <file>` before the call, and after any create or update carrying `@<file>` read the item back — `az boards work-item show --id <id> --output json --query fields` — treating a rich-text field that is empty or begins with `@` as a failed publish: fix the path (absolute is safest), never re-run the same command. **Every rich-text file goes in a per-item directory** — `<scratchpad>/<type>-<slug>/description.html`, `.../acceptance.html`, `.../repro.html` — because every publisher that writes a rich-text file names it `description.html`, `acceptance.html`, or `repro.html`: a `/to-feature` followed by `/to-story --parent`, or a `to-tasks` batch publishing several Tasks, otherwise overwrites the previous body in the window between the `test -s` and the `@` read, and the read-back catches an empty field, never a wrong one.

For an acceptance-criteria field — `to-feature` and `to-story` only; ADO Tasks and Bugs have none, and `chart-course` publishes none — the read-back is `az boards work-item show <id> --output json --query 'fields."Microsoft.VSTS.Common.AcceptanceCriteria"'`; empty or null means the criteria landed in the description instead of the field — patch with `az boards work-item update --id <id> --fields "Microsoft.VSTS.Common.AcceptanceCriteria=@/absolute/path/acceptance.html"` and strip them from the description. Buried criteria aren't queryable, and child items' `Covers:` lines then point at IDs no field holds.

## First item of a batch alone

A batch of creates — a `to-tasks` run, a Feature followed by its Stories — publishes its first item alone, and you open that item as the tracker renders it before the next create goes out. A mangled render (a GitHub body whose markdown broke, the ADO read-back's empty or `@`-prefixed field) stops the batch until the body is fixed and the first item re-checked; only then are the rest created, from the fixed body. Every item created before the check carries the same defect.
