...
description: Writing member-facing copy a person can act on — the term defined where it appears, the amount as the figure billed, not a formula, the next step with a date, and a way to get help. Use when writing or reviewing text a member, patient, or subscriber will read — an error message, an in-app notice or form label, an email, letter, SMS, or EOB template, a denial, appeal, prior-authorization, or coverage-termination notice, a benefits, eligibility, or claim-status explanation, a cost-share breakdown. Also use when copy quotes plan documents or regulation, when a template variable can render empty or zero, when the copy will be translated or a reading-level score is run on a translation, or when a draft reads above the project's target. Also when a bug or finding names a message a member sees. Not for internal-facing text (logs, developer errors, admin tooling) — writing-for-humans'. Not the approved-language list or reading-level policy — the convention skill's.
