...
- **A line that has conflicted before is resolved this time and moved out of PRs.** A version string, a lockfile hunk, or a generated index at a path a prior merge already resolved — `git log --merges -- <path>` shows the earlier resolution, or `git rerere` holds one — is resolved now like any other conflict, and the recurrence is raised as a finding by the route below (the work item § Boundary names, shaped by `work-item-shape`), naming the fix on trunk: bump or regenerate there, so the next merge has nothing to resolve.
