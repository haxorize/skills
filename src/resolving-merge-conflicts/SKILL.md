...
- **A line that conflicts on every merge is resolved once, then moved out of PRs.** A version string, a lockfile hunk, or a generated index that conflicts after each merge to trunk is resolved this once and reported as a follow-up naming the fix on trunk — bump or regenerate there — never resolved again next time.
