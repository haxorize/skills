...
description: Naming and placing code so a plain-text search finds it — identifiers as search queries, whole string literals, one definition site, a doc line the natural-language grep lands on. Use when naming or renaming an exported symbol, type, file, error message, event name, flag, or a field holding an identifier another system minted, when moving code between files, when a search for a concept returns nothing or a wall of hits, when the code deliberately leaves out something a reader would search for, when reviewing a diff for names the next session will not find, or when another skill needs the discoverability vocabulary.
---
...
- **Put the disambiguating context in the symbol, not the folder.** The import that tells `users/diff.ts` from `orders/diff.ts` sits at the top of the file; the search hit is at line 300. Name it `diffUserObjects`. The folder's job is the map: a listing is the first search in an unfamiliar repo, so folders group by feature, domain, or layer and the listing reads as the system's shape — the context a search hit needs still lives in the symbol.
...
- **An identifier's name says who minted it.** A bare `id` is the local record's; one minted elsewhere carries its minter's name (`stripeCustomerId`, `placeEid`) rather than being aliased as the local id, and a record carrying two external identities qualifies both. The bullet above wins where they meet: under a rigid repo convention that fixes the field name (`id` on every mapping table), keep the convention's name and put the minter in the doc line on the definition instead.
...
- **A name something outside the repo has already bound is frozen, not stale.** A user-data directory, an app id, a keychain or `safeStorage` key, a deeplink scheme already posted to users, an env prefix baked into deployed sidecars: renaming one orphans what the old name reaches (a database volume, a stored key) rather than misinforming a reader, so the freeze is written at the definition, the way a deliberate absence is below, naming what breaks on rename.
...
4. Did anything change behavior, audience, or visibility without changing its name, or without the freeze written at its definition?
5. Where code moved, is it gone from where it came from?
6. Where something was renamed, does the old name search to zero, with every remaining hit named as deliberate — and where the rename inverted a sense, does every sentence stating the default agree with the code?
