...
- **An identifier's name says who minted it.** A bare `id` is the local record's; one minted elsewhere carries its minter's name (`stripe_customer_id`, `place_eid`) and is never aliased as the local id, and a record carrying two external identities qualifies both.
